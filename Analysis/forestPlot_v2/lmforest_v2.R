...
#' @export
lmforest_v2 <- function(x, data = NULL,
                     title = "Linear Model",
...
# THE MOST MODIFIED PART -------------------------------------------------
  ...
  p <- ggplot2::ggplot(to_show_exp_clean, ggplot2::aes(x_pos, estimate)) +
    ...
    ggplot2::scale_x_continuous(expand = ggplot2::expansion(mult = c(0.15, 0.05))) +
    ...
    ggplot2::theme(plot.margin = ggplot2::margin(t = 0, r = 0, b = 2, l = 0),
                   ...
                   plot.title = ggplot2::element_text(size = font_size * title_size, face = "bold", hjust = 0.06)) +
    ggplot2::xlab("") +
    
    ## Variable column
    ggplot2::annotate(geom = "text", x = max(to_show_exp_clean$x_pos) + 1.5, y = y_variable,
                      label = "Fixed Effects", fontface = "bold", hjust = 0,
                      size = header_font) +
    ...
    ggplot2::annotate(geom = "text", x = max(to_show_exp_clean$x_pos) + 1.4, y = y_coef,
                      label = paste0("bold('Estimated coefficient (", ci_pct, "% CI); '*bolditalic(p)*'-value')"),
    ...
    ## X-axis label
    ggplot2::annotate(geom = "text", x = -1.5, y = reference_value -0.5,
                      label = "Estimated coefficient (95% CI)",
                      # label = expression("Estimated coefficient" ~ (hat(beta))), fontface = "bold",
                      # hjust = 0.5, vjust = 2, size = annot_font * 1.5
                      hjust = 0.5, vjust = 2, size = annot_font * 1.2) +
    ggplot2::annotate(geom = "text", x = 0.5-5.5, y = y_variable,
                      label = paste0("Observations analyzed: ", gmodel$nobs,
                                     "\nMarginal R\u00b2: ", format_number(gmodel$r_squared, 3, marks), ## r2_str
                                     " /  Conditional R\u00b2: ", format_number(gmodel$conditional_r2, 3, marks), ## round(gmodel$conditional_r2, 3)
                                     "",
                                     "\nAIC: ", gmodel$AIC_formatted),
                      size = annot_font * 0.8, hjust = 0, vjust = 1.2, fontface = "italic") +
    ...
    ggplot2::annotate(geom = "text", x = -0.5, y = y_variable,
                      label = "Random effects", fontface = "bold", hjust = 0,
                      size = header_font) +
    
    ggplot2::annotate(geom = "text", x = -0.5 - c(1, 2, 3, 4), y = y_coef,
                      label = random_effect_label,
                      hjust = 0,
                      size = annot_font) +
    
    ggplot2::annotate(geom = "text", x = -0.5 - c(1, 2, 3, 4), y = y_variable,
                      label = c("hat(sigma)^2", 
                                "hat(tau)['00']^id", 
                                "ICC", 
                                "N[id]"),
                      size = annot_font, hjust = 0, fontface = "italic",
                      parse = T)
